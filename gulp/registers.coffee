...
plugins.gulp.task 'compile:prod', [ 'compile:dev' ],  (done) -> plugins.runSequence 'clean', 'copy', 'usemin', 'deploy', done
...
plugins.gulp.task 'compile:prod', [ 'compile:dist' ], (done) -> plugins.runSequence 'redirect', 'deploy', done
plugins.gulp.task 'serve:dev',    [ 'compile:dev'  ], (done) -> 
    sync.init
        server :
            baseDir : './dev/',
